...
const expandMigration = read("supabase/migrations/20260826120000_anonymous_vote_expand.sql");
const scrubMigration = read("supabase/migrations/20260826160000_anonymous_vote_scrub.sql");
...
if (!authorizeVote.includes("consume_rate_limit") || !authorizeVote.includes("issue_ballot_authorization") || authorizeVote.includes("choice_id")) failures.push("Phase A must rate-limit and authorize without receiving a choice");
if (!votingAccount.includes("auth.getUser(token)") || !votingAccount.includes("admin.passkey.listPasskeys({ userId: user.id })")) failures.push("vote authorization must reuse the validated account and Passkey checks");
if (!submitBallot.includes("redeem_ballot_permit") || !submitBallot.includes('req.headers.has("Authorization")')) failures.push("Phase B must redeem one anonymous permit and reject user Authorization");
if (!expandMigration.includes("unique (user_id, poll_id)") || !expandMigration.includes("insert into public.votes (poll_id, choice_id)")) failures.push("dissociated database participation or ballot invariant missing");
if (!scrubMigration.includes("drop table public.user_poll_answers") || !scrubMigration.includes("drop column if exists user_lock_id")) failures.push("legacy account-to-choice storage must be removed by SCRUB");
