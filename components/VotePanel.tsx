...
type Step = "checking" | "visitor" | "email" | "passkey" | "confirm" | "submitting" | "success" | "duplicate" | "closed" | "error";
...
export function VotePanel({ visible, pollId, choiceId, choiceLabel, platform, onClose, onFinished }: Props) {
  ...
  useEffect(() => {
    ...
    async function resolveStep() {
      ...
      if (!active) return;
      if (profile?.passkey_required_at && !profile.passkey_enrolled_at) setStep("passkey");
  ...
  }, [emailVerified, slide, user, visible]);
  ...
  async function handleSubmit() {
    if (running.current || step !== "confirm") return;
    ...
    <Modal transparent visible={visible} animationType="fade" onRequestClose={closeAll}>
      <View style={styles.overlay}>
        ...
        <Animated.View style={StyleSheet.flatten([styles.panel, animatedStyle])}>
          <ModalHeader title={getTitle(step)} choiceLabel={choiceLabel} onClose={closeAll} />
          <View style={styles.body}>
            ...
            {step === "confirm" ? (
              <>
                <Text style={styles.helperText}>Confirmez votre choix. Une seule participation est possible pour cette question.</Text>
              ...
              <View style={styles.successBody}>
                <Text accessibilityLiveRegion="polite" style={styles.successText}>
                  {resultsRefreshFailed
                    ? "Votre vote a bien été enregistré, mais les résultats ne peuvent pas être actualisés pour le moment."
                    : "Merci d'avoir pris part au débat. Votre vote a bien été comptabilisé. Vous pouvez maintenant suivre l'évolution des résultats."}
                </Text>
                {resultsRefreshFailed ? <AnimatedPrimaryButton label="Réessayer d’actualiser" onPress={retryResultsRefresh} /> : null}
                <Pressable accessibilityRole="button" onPress={() => go("/themes" as Href)} style={styles.successPrimary}>
                  <Text style={styles.successPrimaryText}>Découvrir les autres sujets</Text>
                </Pressable>
              </View>
            ) : null}
            ...
            {step === "error" ? (
              <>
                ...
                <View style={styles.actions}>
                  <AnimatedPrimaryButton label="Réessayer" onPress={() => setStep("confirm")} />
...
function ModalHeader({ title, choiceLabel, onClose }: { title: string; choiceLabel: string; onClose: () => void }) {
  return (
    <View style={styles.header}>
      ...
      <Pressable accessibilityRole="button" accessibilityLabel="Fermer" onPress={onClose} style={styles.iconButton}><X size={18} color={palette.inkSecondary} /></Pressable>
    </View>
  );
}

function AnimatedPrimaryButton({ label, loading, disabled, onPress }: { label: string; loading?: boolean; disabled?: boolean; onPress: () => void }) {
  const [active, setActive] = useState(false);
  const fill = useMemo(() => new Animated.Value(0), []);
  function animate(toValue: number) {
    if (disabled) return;
    setActive(toValue === 1);
    Animated.timing(fill, { toValue, duration: 220, easing: Easing.out(Easing.cubic), useNativeDriver: true }).start();
  }
  return (
    <Pressable
    ...
    >
      <Animated.View pointerEvents="none" style={StyleSheet.flatten([styles.primaryFill, { transform: [{ translateY: fill.interpolate({ inputRange: [0, 1], outputRange: [52, 0] }) }] }])} />
      {loading ? <ActivityIndicator color={active ? palette.onPrimary : palette.primaryStrong} /> : <Text style={StyleSheet.flatten([styles.primaryText, active && styles.primaryTextActive])}>{label}</Text>}
    </Pressable>
  );
}
...
const styles = StyleSheet.create({
  ...
  scrim: { ...StyleSheet.absoluteFillObject, backgroundColor: "rgba(2, 6, 23, 0.66)" },
  panel: { width: "100%", maxWidth: 390, borderRadius: radius.md, backgroundColor: palette.surface, borderWidth: 1, borderColor: palette.line, padding: 22, gap: 14, ...shadows.panel },
  header: { flexDirection: "row", justifyContent: "space-between", alignItems: "flex-start", gap: 12, paddingBottom: 12, borderBottomWidth: 1, borderBottomColor: palette.line },
  ...
  iconButton: { width: 32, height: 32, borderRadius: radius.sm, alignItems: "center", justifyContent: "center", backgroundColor: palette.surfaceRaised },
  ...
  primaryButton: { minHeight: 44, borderRadius: radius.sm, borderWidth: 1, borderColor: palette.primaryStrong, backgroundColor: "transparent", alignItems: "center", justifyContent: "center", flexDirection: "row", overflow: "hidden", paddingHorizontal: 16 },
  ...
  secondaryButton: { minHeight: 44, borderRadius: radius.sm, paddingHorizontal: 14, alignItems: "center", justifyContent: "center", borderWidth: 1, borderColor: authField.separatorColor },
  ...
  successPrimary: { minHeight: 44, borderRadius: radius.sm, backgroundColor: palette.primary, alignItems: "center", justifyContent: "center", paddingHorizontal: 16 },
